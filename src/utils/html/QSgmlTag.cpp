...
QString QSgmlTag::toString(bool recursive) const {
    QString prefix = "QSgmlTag(";
    QString suffix = "\n)";
    QString id = Name + " " + Value + "\n\t";
    QString attrs;
    for (const QString key : Attributes.keys()) {
        attrs += key + "=" + Attributes.value(key) + ", ";
    }

    return prefix + id + attrs + suffix;
}

QString QSgmlTag::getText() const {
    if (Name == "")
        return Value;
    for (const QSgmlTag* child : Children) {
...
QSgmlTag::QSgmlTaglist QSgmlTag::getElementsByName(const QString name) {
    QSgmlTaglist list;
    ...
    for (QSgmlTag* child : Children)
        list.append(child->getElementsByName(name));
...
QSgmlTag* QSgmlTag::find(const QString name, const QString attrName, const QString attrValue) {
    ...
    if (Name == name && checkAttribute(attrName, attrValue))
        return this;

    for (QSgmlTag* child : Children) {
        elem = child->find(name, attrName, attrValue);
        if (elem != nullptr)
            break;
    }
    return elem;
